...
with st.sidebar.expander("🛠️ Configuration Panel", expanded=st.session_state.config_expanded):
    ...
    ai_provider = st.selectbox(
        "Choose AI Provider", 
        ["OPENAI", "GEMINI", "OLLAMA","DEEPSEEK"],  # Add OLLAMA here
        index=["OPENAI", "GEMINI", "OLLAMA","DEEPSEEK"].index(os.getenv("AI_PROVIDER", "OPENAI").upper())
    )
    ...
    if ai_provider == "OPENAI":
        ...
        ollama_model = st.text_input("Ollama Model", value=os.getenv("OLLAMA_MODEL", "llama3"))
        openai_api_key = ""
        openai_model = ""
        gemini_api_key = ""
        gemini_model = ""
        ...
        deepseek_api_key = st.text_input("Deepseek API Key", value=os.getenv("DEEPSEEK_API_KEY", ""), type="password")
        deepseek_model = st.text_input("Deepseek Model", value=os.getenv("DEEPSEEK_MODEL", "deepseek-default-model"))
        openai_api_key = ""
        openai_model = ""
        gemini_api_key = ""
        gemini_model = ""
    ...
    if st.button("💾 Save Configuration"):
        ...
        # Save keys based on selected provider, clear others
        if ai_provider == "OPENAI":
            ...
            set_key(dotenv_path, "DEEPSEEK_MODEL", "")
        elif ai_provider == "GEMINI":
            ...
            set_key(dotenv_path, "OLLAMA_MODEL", ollama_model)
            set_key(dotenv_path, "OPENAI_API_KEY", "")
            set_key(dotenv_path, "OPENAI_MODEL", "")
            set_key(dotenv_path, "GEMINI_API_KEY", "")
            set_key(dotenv_path, "GEMINI_MODEL", "")
            ...
            set_key(dotenv_path, "DEEPSEEK_API_KEY", deepseek_api_key)
            set_key(dotenv_path, "DEEPSEEK_MODEL", deepseek_model)
            set_key(dotenv_path, "OPENAI_API_KEY", "")
            set_key(dotenv_path, "OPENAI_MODEL", "")
            set_key(dotenv_path, "GEMINI_API_KEY", "")
            set_key(dotenv_path, "GEMINI_MODEL", "")
